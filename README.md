...
written back to HubSpot, persisted, and — for Hot leads — posted to Slack.
...
| `mcp/` | MCP server (`server.py`) exposing `list_hot_leads`/`get_qualification` as tools — same `LeadRepositoryPort` the HTTP API uses, so results never diverge between the two surfaces. |
...
A `Dockerfile`/`docker-compose.yml` also exist for a container-based deploy path (e.g. Cloud Run) if you ever move off FastAPI Cloud — note FastAPI Cloud itself ignores both and builds directly from `pyproject.toml`/`uv.lock`.
